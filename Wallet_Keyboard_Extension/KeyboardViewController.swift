...
class KeyboardViewController: UIInputViewController {
    ...
    private let walletAddress = "7avUQL5L49iF6PY7iUwuTxYVsySwYf3U3ucjTEdtWzyf"
    
    private let linkQueue = [
    ...
    private let explorerURL = "https://solscan.io/account/"
    ...
    @objc private func sendAmount() {
        guard let link = linkQueue.first else {
            insertText("No more links available for Solana.")
            return
        }
    ...
    @objc private func requestAmount() {
        ...
        DispatchQueue.main.asyncAfter(deadline: .now() + 0.8) { [weak self] in
            ...
            let link = "https://amanu.app/claim/request?rea=\(self.walletAddress)&amt=\(formattedAmount)&chain=solana"
            self.insertText(link)
    ...
    @objc private func addressTapped() {
        let fullExplorerURL = explorerURL + walletAddress
        if let url = URL(string: fullExplorerURL) {
            openURL(url)
        }
